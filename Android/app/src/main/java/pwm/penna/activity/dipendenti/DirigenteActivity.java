...
public class DirigenteActivity extends ListActivity implements AsyncActivity {
    private static final String TAG = "DirigenteActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((TextView) findViewById(R.id.title)).setText(R.string.dirigenti);
    }
    ...
    public void onResume() {
        ...
        AsyncReq.executeEndpoint(this, "Dipendenti?q=dirigenti");
    }

    @Override
    public void setResult(JSONObject jsonObject) {
        DipendenteJSONDAOImpl dipendenteJSON = new DipendenteJSONDAOImpl();
        ArrayList<Dirigente> dipendenti = dipendenteJSON.getDirigenti(jsonObject);

        setListAdapter(new ArrayAdapter<>(this, R.layout.item_view, R.id.textView, dipendenti));

        getListView().setOnItemClickListener((parent, view, position, id) -> {
            Dirigente d = (Dirigente) parent.getItemAtPosition(position);

            Intent intent = new Intent(this, DipendenteActivity.class);
            intent.putExtra(DipendenteActivity.TAG, d);
